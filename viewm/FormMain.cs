							...
							Thread thread = new Thread(() =>
								{
									...
									ChunkProcessor chunkProcessor = new ChunkProcessor(directories[index]);
									...
									chunkProcessor.ProcessComplete += () =>
										{
											...
											BeginInvoke(new Action(() =>
												{
													labelStatus.Text = "Process completed in " + (eTime - sTime).Seconds + " seconds.";
									...
									chunkProcessor.ProgressChanged += (p) => BeginInvoke(new Action(() =>
										{
